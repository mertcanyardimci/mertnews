...
<div class="row">
  <div class="col-md-4">
    <div class="card shadow mb-4">
      <div class="card-header pu-3">
        <h6 class="m-0 font-weight-bold text-primary">Yeni Yazar Ekle</h6>
      </div>
      <div class="card-body">
        <form method="POST" action="{{route('admin.yazars.create')}}">
          @csrf
  ...
  <div class="col-md-7">
    <div class="card shadow mb-4">
      <div class="card-header pu-3">
        <h6 class="m-0 font-weight-bold text-primary">Tüm Yazarlar</h6>
      </div>
      <div class="card-body">
                  <!-- Partial Table -->
                ...
                @foreach($yazars as $yazar)
                  <tr>
                    ...
                    <td class="fw-semibold fs-sm">
                      <a target="_blank">{{$yazar->name}}</a>
                    </td>
                    <td class="d-none d-sm-table-cell">
                      <span class="fs-xs fw-semibold d-inline-block py-1 px-3 rounded-pill bg-success-light text-success"><a>{{$yazar->newsCount()}}</a></span>
                    </td>
                    <td class="text-center">
                    <a yazar-id="{{$yazar->id}}" class="btn btn-sm btn-primary edit-click" title="Düzenle"><i class="fa fa-edit"></i></a>
                    <a yazar-id="{{$yazar->id}}"  yazar-count="{{$yazar->newsCount()}}"yazar-name="{{$yazar->name}}" class="btn btn-sm btn-danger remove-click" title="Sil"><i class="fa fa-remove"></i></a>
                    </td>
                  </tr>
                @endforeach
                </tbody>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h1 class="modal-title">Yazarı Düzenle</h1>
        <button type="button" class="close" data-bs-dismiss="modal">&times;</button>
      </div>
      <div class="modal-body">
        <form method="POST" action="{{route('admin.yazars.update')}}">
          @csrf
          <div class="form-group">
            <label>Yazar Adı:</label>
            <input type="text" class="form-control" id="yazar" name="yazar"/>
            <input type="hidden" id="yazar_id" name="id"/>
          </div>
      </div>
...
<div class="modal" id="deleteModal">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h1 class="modal-title">Yazarı Sil</h1>
        <button type="button" class="close" data-bs-dismiss="modal">&times;</button>
      </div>
      ...
      <div class="modal-footer">
        ...
        <form method="POST" action="{{route('admin.yazars.delete')}}">
        @csrf
...
<script>
  $(function() {
    $('.edit-click').click(function(){
      id = $(this)[0].getAttribute('yazar-id');
      $.ajax({
        type:'GET',
        url:"{{route('admin.yazars.getdata')}}",
        data:{id:id},
    ...
    $('.remove-click').click(function(){
      id = $(this)[0].getAttribute('yazar-id');
      count = $(this)[0].getAttribute('yazar-count');
      name = $(this)[0].getAttribute('yazar-name');
      if(id==1){
      ...
      $('#body').hide();
      if(count>0){
        $('#newsAlert').html('Bu yazara ait '+count+' haber bulunmaktadır. Silmek istediğinize emin misiniz ?');
        $('#body').show();
      }
      if(count==0){
