...
<div class="card shadow mb-4">
    ...
    <div>&nbsp&nbsp&nbsp</div>
        <h1 class="m-3 font-weight-bold text-primary">Haberi Güncelle</h1>
    </div>
    <div class="card-body">
      @if($errors->any())
      ...
      @endif
        <form method="POST" action="{{route('admin.haberler.update',$news->id)}}" enctype="multipart/form-data">
          @method('PUT')
          @csrf
            <div class="form-group">
                <label>Haber Başlığı</label>
                <input type="text" name="title" class="form-control" value="{{$news->title}}" required></input>
            </div>
            <div>&nbsp&nbsp&nbsp</div>
            <div class="form-group">
                ...
                <select class="form-control" name="yazar" required>
                    @foreach($yazars as $yazar)
                      <option @if($news->yazar_id==$yazar->id) selected @endif value="{{$yazar->id}}">{{$yazar->name}}</option>
                    @endforeach
                </select>
            </div>
            <div>&nbsp&nbsp&nbsp</div>
            <div class="form-group">
                <label>Haber Fotoğrafı</label><br />
                <img src="{{($news->image)}}" class="img-thumbnail rounded" width="300" />
            ...
            <div>&nbsp&nbsp&nbsp</div>
            <div class="form-group">
                <button type="submit" class="btn btn-primary btn-block">Haberi Güncelle</button>
            </div>
        </form>
    </div>
      <!-- Footer -->
